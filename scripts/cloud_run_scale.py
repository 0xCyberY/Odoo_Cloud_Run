...
SERVICE_NAMES = {
    "pooled": "pooled-odoo",
    "cron-runner": "cron-runner-odoo",
}


def scale_service(service, min_instances, region, project):
    # All clients are pooled — a service, not a client, is the scaling unit.
    service_name = SERVICE_NAMES[service]
    print(f"Scaling '{service_name}' (min-instances={min_instances})...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--service", default="pooled", choices=sorted(SERVICE_NAMES),
                         help="Which shared service to scale (default: pooled). "
                              "'cron-runner' is what README's pause/resume-during-bootstrap steps target.")
    ...
    try:
        scale_service(args.service, args.min_instances, args.region, args.project)
